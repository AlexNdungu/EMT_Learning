...
function login_user(){
    ...
    formData.append('email',log_email.value);
    ...
    $.ajax({
        ...
        success: function(response){
            if(response.status == 'not_found'){
                new Alert.Alert('error','User Not Found!',alert_time,alert_section);
            }
...
function check_empty_inputs(email,pass){
    if(email.value == ''){
        new Alert.Alert('error','Email Is Emplty!',alert_time,alert_section);
    }
    else{
        if(pass.value == ''){
            new Alert.Alert('error','Password Is Emplty!',alert_time,alert_section);
        }
        else{
...
login_btn.addEventListener('click', ()=> {
    check_empty_inputs(log_email,pass_1)
});